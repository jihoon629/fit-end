...
public class FoodApi {
        ...
        @GetMapping("/up")
        @Operation(summary = "CSV 데이터 저장", description = "내부적으로 CSV에서 신체 데이터를 읽고 저장하는 API입니다. 중복 저장 방지 기능이 추가되었습니다.", responses = {
                        @ApiResponse(responseCode = "200", description = "CSV 데이터 저장 성공"),
                        @ApiResponse(responseCode = "409", description = "이미 데이터가 저장되어 있음"),
                        @ApiResponse(responseCode = "500", description = "서버 내부 오류")
        })
        public ResponseEntity<String> importFemaleScoreRank() {
                try {
                        // 클래스패스에서 CSV 파일 경로 가져오기
                        String csvFilePath = new ClassPathResource("bodydata.csv").getFile().getPath();
                        SaveRawFood.importFemaleScoreRankData(csvFilePath);
                        return ResponseEntity.ok("여성 점수 랭크 데이터 가져오기 성공");
                } catch (IOException | CsvValidationException e) {
                        e.printStackTrace();
                        return ResponseEntity.badRequest().body("데이터 가져오기 실패: " + e.getMessage());
                }
        }

        @GetMapping("/foodname/{foodNm}") // 음식 이름으로 검색하는 컨트롤러
        @Operation(summary = "음식 이름으로 검색", description = "사용자가 입력한 음식 이름을 기반으로 영양 정보를 검색합니다.", responses = {
                        @ApiResponse(responseCode = "200", description = "음식 정보 조회 성공", content = @Content(array = @ArraySchema(schema = @Schema(implementation = FoodDto.class)))),
                        @ApiResponse(responseCode = "404", description = "해당 음식 정보 없음")
        })
        public ResponseEntity<List<FoodDto>> FoodName(
                        @Parameter(description = "검색할 음식 이름", example = "김치") @PathVariable String foodNm) {
        ...
        @Operation(summary = "음식 기록 저장", description = "사용자의 음식 섭취 기록을 저장합니다.", responses = {
        ...
        @Operation(summary = "사용자 식단 기록 조회", description = "사용자의 식단 기록을 조회하는 API입니다.", responses = {
                        @ApiResponse(responseCode = "200", description = "식단 기록 조회 성공", content = @Content(array = @ArraySchema(schema = @Schema(implementation = DietRecord.class)))),
                        @ApiResponse(responseCode = "404", description = "사용자 식단 기록 없음")
        })
        public ResponseEntity<List<DietRecord>> getUserDietRecords(
                        @Parameter(description = "사용자 ID", example = "testuser") @PathVariable String userid) {
