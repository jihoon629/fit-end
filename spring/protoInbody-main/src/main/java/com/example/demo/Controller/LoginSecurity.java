...
public class LoginSecurity {
        ...
        @Operation(summary = "로그인", description = "사용자 아이디와 비밀번호를 검증하여 로그인 처리합니다. 로그인 시도 횟수를 관리하며, 성공 시 JWT 토큰을 발급합니다.", responses = {
        ...
        @Operation(summary = "로그아웃", description = "사용자의 JWT 토큰을 만료시켜 로그아웃 처리합니다.", responses = {
                        @ApiResponse(responseCode = "200", description = "Logged out successfully", content = @Content(schema = @Schema(implementation = Map.class)))
        })
        @PostMapping("/logout")
        ...
        @Operation(summary = "토큰 검증", description = "요청에 포함된 JWT 토큰을 검증하여 사용자의 아이디를 반환합니다.", responses = {
                        @ApiResponse(responseCode = "200", description = "유효한 토큰", content = @Content(schema = @Schema(implementation = Map.class))),
        ...
        public ResponseEntity<?> validateToken(HttpServletRequest request) {
                ...
                int sex = UserBodyInfoService.findsex(userid);

                if (userid != null) {
                        {
                                {
                                        return ResponseEntity.ok(Map.of("userid", userid, "sex", sex)); // 성별 정보도 포함하여
                                                                                                        // 반환 }}
                                }
                        }
                } else {
